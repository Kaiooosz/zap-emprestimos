  ...
  useEffect(() => {
    async function fetchClient() {
      try {
        ...
        setTipo(data.tipo);
        setTemContrato(data.temContrato);
        setGarantia(data.garantia);
        setPerfilTaxa(data.perfilTaxa || "10");
        setTaxaPadrao(data.taxaPadrao ? String(data.taxaPadrao) : "10");
        setForm({
          nome: data.nome || "",
          cpf: data.cpf || "",
          cnpj: data.cnpj || "",
          ...
          rendaMensal: data.rendaMensal ? String(data.rendaMensal) : "",
          referencia: data.referencia || "",
          observacoes: data.observacoes || "",
          tipoGarantia: (data.tipoGarantia as TipoGarantia) || "IMOVEL",
          valorGarantia: data.valorGarantia ? String(data.valorGarantia) : "",
          descricaoGarantia: data.descricaoGarantia || "",
        });
      } catch (e) {
        console.error(e);
      }
  ...
  return (
    <div className="space-y-5 max-w-3xl">
      <div className="flex items-center gap-3">
        <Link href={`/clientes/${id}`} className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-400 hover:border-slate-500 hover:text-white transition-colors">
          <ArrowLeft size={15} />
        </Link>
        <div>
          <h1 className="text-base font-semibold text-slate-900 tracking-tight">Editar Cliente</h1>
        </div>
      </div>
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="flex justify-end gap-3 border-t border-slate-100 pt-5">
          <Link href={`/clientes/${id}`} className="rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-colors">Cancelar</Link>
          <button type="submit" disabled={loading} className="rounded-xl bg-blue-700 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-800 disabled:opacity-50 transition-all">
